...
| [Dispersions](docs/gui/Dispersions.md)   | See two sensors against each other, the way the historian's interpolation shows up in a scatter plot | A point cloud, with its density, of two sensors over a scope (every instance, one class or one well), colored by class, well or label period, thinnable to genuine measurements alone, or one dot per instance                        |
